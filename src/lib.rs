...
mod tests {
    ...
    fn transfer_sol() {
        let keypair = read_keypair_file("dev-wallet.json").expect("Couldn't find wallet file");
        let to_pubkey = Pubkey::from_str("H2ZFj5AwnN18iCqMgXsuohgU5GWTmEEJ6ahtE1es5F8C").unwrap();
    ...
    fn wba_prereq() {
        ...
        let args = CompleteArgs { github: b"cb7chaitanya".to_vec() };
        ...
        let tx = TurbinePrereqProgram::complete(
            ...
            &[&signer],
            blockhash );
        let signature = rpc_client .send_and_confirm_transaction(&tx).expect("Failed to send transaction");
